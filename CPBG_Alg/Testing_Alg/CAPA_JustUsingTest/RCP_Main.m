function [Oskeleton,SADAcell] = RCP_Main(X,data,Cskeleton,idx,SADAcell,skeleton,Ds_Temp,t)
t = t + 1;
Oskeleton = Cskeleton;
[n,d]=size(X);
...
%     aaaaaaaaaaaaaa  = idx
    ...
    [idxA,idxB,idxCut ,Pa,Pb,Pc]=RCP_Split_final(X,Ds_Temp,idx); 
...
%         bbbbbbbbbbbbbbbbb  =  unique([Ra,Rb,Rc])
        ...
        if length(Ra) ~= length(intersect(Ra,Rc)) && length(Ra) >= 2
            [Oskeleton,SADAcell] = RCP_Main( data(:, Ra),data,Oskeleton,Ra,SADAcell,skeleton,Ds_Temp,t);
        end
        if length(Rb) ~= length(intersect(Rb,Rc)) && length(Rb) >= 2
            [Oskeleton,SADAcell] = RCP_Main( data(:, Rb),data,Oskeleton,Rb,SADAcell,skeleton,Ds_Temp,t);
        end
        % 对D进行了分割
        if length(Rc) >= 2 % D分离集不能大于2？？？
            [Oskeleton,SADAcell] = RCP_Main( data(:, Rc),data,Oskeleton,Rc,SADAcell,skeleton,Ds_Temp,t);
        end
    end
end
