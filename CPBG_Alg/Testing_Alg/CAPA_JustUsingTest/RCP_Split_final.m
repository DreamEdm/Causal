function [idxA,idxB,idxCut,Pa,Pb,Pc]=RCP_Split_final(X,Ds_Temp,idxGloble)
[~, nNode]=size(X);
...
%     else
    [idxA,idxB,idxCut,Pa,Pb,Pc]=SADA_Split_Basic(X(:, 1:nNode),Ds_Temp,idxGloble(1:nNode));
%     end
end

function [idxA,idxB,idxCut,Pa,Pb,Pc]=SADA_Split_Basic(X,Ds_Temp,idxNow)
[~, nNode]=size(X);
idxA=false(nNode, 1);
idxB=false(nNode, 1);
idxCut=true(nNode, 1);
...
% function [idxA,idxB,idxCut,Pa,Pb,Pc]=SADA_Split_Basic2(X,Ds_Temp,idxNow)
% [~, nNode]=size(X);
% idxA=false(nNode, 1);
% idxB=false(nNode, 1);
% idxCut=true(nNode, 1);
...
%     end
% end
% beforecccccccccc = [idxNow',idxA,idxCut,idxB]
% %--------------------------------------------------------------------------
% flag = true;
% maxIter = 1000000;
% while flag == true
%      midumccccccc = [idxNow',idxA,idxCut,idxB]
%     Ds = DsHold;
%     idA = find(idxA == 1)';
%     idB = find(idxB == 1)';
%     idC = find(idxCut == 1)';
...
%                 end
%                 tSet = MC(i,:);
%                 idxReplace = setdiff(1:length(idA),find(sum(Ds(tSet,idA))==k));
%                 idxRepB = setdiff(1:length(idB),find(sum(Ds(tSet,idB))==k));
%                 min2 = min(length(idxReplace),length(idxRepB));
%                 if min2 < minReplace
%                     minReplace = min2;
%                 end
...
%     end
% end
% aftercccccccccc = [idxNow',idxA,idxCut,idxB]
% %--------------------------------------------------------------------------
% 
% idxC = true(nNode, 1);
% TempAB = [find(idxA == true)',find(idxB == true)'];
