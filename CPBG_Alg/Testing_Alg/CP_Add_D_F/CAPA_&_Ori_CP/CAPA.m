function [RCP_skeleton,Time_CAPA_Partition,Time_PC_CAPA,originalSize_currentSize_maxSubsetSize] = CAPA(data,skeleton,conSize,Ds_order,Ds_Temp)
...
%      Time_Ds_Temp_ReCIT = toc
    ...
    tic
    [RCP_skeleton,RCP_Cell] = CAPA_Main(data,data,RCP_skeleton,1:node,RCP_Cell,skeleton,Ds_Temp,0); % here par 0 used to limit the recusive time
    Time_CAPA_Partition = toc
    %------------------------remove redundance-------------------------------------------------
    tic
    Size_Of_RCP_Cell =[];
    c = size(RCP_Cell,2);
    ...
    [~,b] = sort(Size_Of_RCP_Cell,'descend');
    Tmep_Cell = RCP_Cell;
    RCP_Cell = {};
...
%     删除重复的子图，如果一个图中完全含有其他图，则将小图忽略
%     gregreg
    %-------------------------------------------------------------------------------------------    
    for i = 1:node
        for j = 1:node
            if Ds_Temp(i,j) == 1     % 如果Ds_Temp表明独立，则令RCP_skeleton为0
                RCP_skeleton(i,j) = 0;
                RCP_skeleton(j,i) = 0;
            end 
        end
    end
    X_skeleton = RCP_skeleton;
    %---------------------------------RCP_KCI----------------------------------------------------
    tic
%     for k = 1:size(RCP_Cell,2) % 20200306
