function [AVG_CPBG_ratio] = Different_E_step_test_f(count)
    Times = 10;
    E_param = [2:0.05:5];
    CPBG_ratio = zeros(Times, size(E_param, 2));
    ...
    parfor Time = 1:Times
        ...
        if size(split_size, 2) > 1
            [rough_graph, skeleton, names, data, idx] = PreEliminate(...
                G, rough_graph, skeleton, names, data, idx);
                [CI_table_2] = GetCITable(rough_graph)
        end
        % ------------------------- CPBG -------------------------
        G = graph(rough_graph, names);
        G_r = graph(rough_graph);
        
%         CPBG_ratio_eachPar = ReturnRatioMatrix_DifferentParam()
        CPBG_ratio_EachPar = zeros(1, size(E_param))
        for E_param_select = 1:size(E_param, 2)
            [Split_graph_Cell, VertexCutSet] = GraphSplitPathSetG_DifferentParam(G, G, names, E_param(E_param_select));
            [CPBG_Cell] = TransformToArray(Split_graph_Cell, G);
            L = size(CPBG_Cell, 2);
            Cell_len = zeros(1, L);
            for i = 1:L
            ...
            CPBG_ratio_EachPar(1, E_param_select) = max(Cell_len)/size(skeleton, 1);
        end
        CPBG_ratio(Time, :) = CPBG_ratio_EachPar;
...
%         CP_Score(Time, :) = ScoreSkeleton(CP_skeleton, skeleton);
    ...
    AVG_CPBG_ratio = zeros(1, kk);
    for kk = 1:size(E_param)
        AVG_CPBG_ratio(:, kk) = sum(CPBG_ratio(:, kk)) / Times;
    end
