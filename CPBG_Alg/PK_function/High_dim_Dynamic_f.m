function [AVG_CPBG_Score, AVG_CAPA_Score, AVG_CP_Score, ...
    ...
    AVG_CPBG_ratio, AVG_CAPA_ratio, AVG_CP_ratio] = High_dim_Dynamic_f(count)
    Times = 10;
    Base_nums = [2.3, 2.5, 2.7, 2.9, 3.0, 3.5, 3.8, 4.2];
    sizeBa = size(Base_nums);
    AVG_CPBG_Score = zeros(Base);
    ...
    for Ba = 1:size(Base_nums)
        Base_num = Base_nums(Ba);
        parfor Time = 1:Times
            [skeleton, names] = LoadingDataset_HiDim(count);
...
%         end
        ...
        CAPA_ratio(Time, 1) = max([length(Ra),length(Rb)])/size(skeleton, 1);
        CAPA_Cell = {};
