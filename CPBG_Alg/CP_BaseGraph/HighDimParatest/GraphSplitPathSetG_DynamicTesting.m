function [Split_graph_Cell, VertexCutSet] = GraphSplitPathSetG_DynamicTesting(G, O_G, names, Base_num)
%GRAPHSPLITPATHSETG This is an equivalent algorithm, and the passed-in parameter is G
    ...
    if ~Split_flag
        ...
        if ~Split_flag && numnodes(G) > 1
            [Split_graph_Cell, VertexCutSet] = PathSplit_PSG_DynamicTesting(G, X, Y, names, Base_num);
        end
    end
end
