function [Split_graph_Cell, VertexCutSet] = GraphSplitPathSetG_DifferentParam(G, O_G, names, E_param)
    ...
    if ~Split_flag
        ...
        if ~Split_flag && numnodes(G) > 1
            [Split_graph_Cell, VertexCutSet] = PathSplit_PSG_DifferentParam(G, X, Y, names, E_param);
        end
    end
end
